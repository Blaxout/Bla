...
ui_texts = {
    'es': {
        "tools_prompt": "WIKI-HERRAMIENTAS > ",
        "installing": "INSTALANDO...",
        "invalid_option": "[!] Opción invalida > ",
        "continue_prompt": "PRESIONA ENTER PARA CONTINUAR...",
        "categories_prompt": "HERRAMIENTAS-CATEGORIAS > ",
        "installing_dots": "Instalando....",
        "main_prompt": "\t\t   WIKI-HERRAMIENTAS > "
    },
    'fr': {
        "tools_prompt": "WIKI-OUTILS > ",
        "installing": "INSTALLATION...",
        "invalid_option": "[!] Option invalide > ",
        "continue_prompt": "APPUYEZ SUR ENTRÉE POUR CONTINUER...",
        "categories_prompt": "OUTILS-CATÉGORIES > ",
        "installing_dots": "Installation....",
        "main_prompt": "\t\t   WIKI-OUTILS > "
    }
}
...
class main:
  def install_tools(self):
    while True:
      ...
      print("")
      logo.back()
      cmd=input(f"\033[1;31m{t['tools_prompt']}\033[00m")
      if cmd=="99" or cmd=="exit":
        ...
        try:
          if int(cmd)>=1 and int(cmd)<=int(total):
            print(f"\033[01;33m{t['installing']}\033[00m")
            tool.install(tool.names[int(cmd)-1])
          else:
            print(f"\007\033\033[1;31m{t['invalid_option']}\033[00m",cmd)
            sleep(1)
        except ValueError:
          print(f"\033[1;31m{t['invalid_option']}\033[00m",cmd)
          sleep(1)
  ...
  def category(self):
    while True:
      ...
      print("")
      logo.back()
      cmd=input(f"\033[1;31m{t['categories_prompt']}\033[00m")
      if cmd=="99" or cmd=="exit":
        self.menu()
        break
      else:
        try:
          if int(cmd) in range(1,int(total)+1):
            while True:
              ...
              tcmd=input(f"\033[1;31m{t['categories_prompt']}\033[00m")
              if tcmd=="99" or tcmd=="exit":
                ...
                try:
                  ...
                  if int(tcmd) in range(1,int(cat_total)+1):
                    print(f"\033[01;32m{t['installing_dots']}\033[00m")
                    tool.install(tmp_cat_tool[int(tcmd)-1])
                  else:
                    print(f"\007\033[1;31m{t['invalid_option']}\033[00m",cmd)
                    sleep(1)
                except ValueError:
                  print(f"\007\033[1;31m{t['invalid_option']}\033[00m",cmd)
                  sleep(1)
  ...
  def menu(self):
    while True:
      ...
      cmd=input(f"\033[1;31m{t['main_prompt']}\033[00m")
      if cmd == "1":
...
class tools:
  ...
  def __init__(self):
    system=sys()
    with open(f"{lang_path}/menu/menu_3/dates/data.json") as data_file:
      self.data=json.load(data_file)
    with open(f"{lang_path}/menu/menu_3/dates/cat.json") as cat_file:
    ...
    if system.connection:
      ...
      if package_manager=="package_manager":
        if os.path.exists(system.bin+"/"+package_name):
          ...
          tmp=input(f"\033[1;33m{t['continue_prompt']}\033[00m")
        else:
          if system.sudo != None:
